...
static bool apply_event(struct telos_session_machine *machine,
                        uint64_t sequence,
                        const char *type,
                        const struct telos_value *payload)
{
    ...
    bool result =
        event != NULL && telos_session_machine_apply(machine, event, NULL);
...
bool telos_zephyr_run_static_scenario(void)
{
    ...
    result = result && machine != NULL && empty != NULL && tool_count != NULL &&
             echo_output != NULL;
    for (size_t index = 0; result && index < sizeof(events) / sizeof(events[0]);
         ++index) {
...
static void find_ipv4_address(struct net_if *interface,
                              ...
                              void *context)
{
    ...
    (void)interface;
    if (!address->found &&
        net_addr_ntop(NET_AF_INET, &interface_address->address.in_addr,
                      address->buffer, address->size) != NULL) {
        address->found = true;
    }
}
...
static bool dns_probe(uint32_t timeout_milliseconds)
{
    static const uint8_t query[] = {
        0x54, 0x45, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x09, 'l',  'o',  'c',  'a',  'l',
        'h',  'o',  's',  't',  0x00, 0x00, 0x01, 0x00, 0x01,
    };
    ...
    if (socket < 0 || zsock_inet_pton(AF_INET, CONFIG_TELOS_NETWORK_PROBE_IPV4,
                                      &server.sin_addr) != 1) {
        if (socket >= 0) {
            zsock_close(socket);
        }
    ...
    if (zsock_sendto(socket, query, sizeof(query), 0,
                     (const struct sockaddr *)&server,
                     sizeof(server)) != (ssize_t)sizeof(query) ||
        zsock_poll(&poll_descriptor, 1, (int)timeout_milliseconds) <= 0) {
        zsock_close(socket);
        return false;
    }
...
bool telos_zephyr_network_wait(uint32_t timeout_milliseconds,
                               char *address,
                               size_t address_size)
{
#if defined(CONFIG_TELOS_NETWORK) && defined(CONFIG_NET_SOCKETS)
