...
struct telos_capability_broker *telos_capability_broker_create(
...
)
{
    ...
    if (
        (available_capability_count > 0 && available_capabilities == NULL)
        || evaluate == NULL
    ) {
        set_error(
            error,
    ...
    if (available_capability_count > 0) {
        ...
        for (
            size_t index = 0;
            index < available_capability_count;
            ++index
        ) {
            broker->capabilities[index] = copy_string(
...
void telos_capability_broker_destroy(
    struct telos_capability_broker *broker
)
{
    if (broker == NULL) {
...
static bool capability_available(
    const struct telos_capability_broker *broker,
...
static bool value_matches_type(
...
static bool validate_schema(
    const struct telos_value *schema,
    const struct telos_value *value,
    unsigned int depth
)
{
    ...
    if (
        schema == NULL
        || telos_value_type(schema) != TELOS_VALUE_OBJECT
        || value == NULL
        || depth > 32
    ) {
        return false;
    ...
    if (strcmp(type, "object") == 0) {
        const struct telos_value *required = telos_value_get(
            schema,
            "required"
        );
        const struct telos_value *properties = telos_value_get(
            schema,
            "properties"
        );
        ...
        for (size_t index = 0; index < telos_value_count(required); ++index) {
            const char *name = telos_value_string(
                telos_value_at(required, index)
            );
        ...
        if (properties != NULL) {
            ...
            for (
                size_t index = 0;
                index < telos_value_count(properties);
                ++index
            ) {
                const char *name = telos_value_key_at(properties, index);
                const struct telos_value *member = telos_value_get(
                    value,
                    name
                );
                ...
                if (
                    member != NULL
                    && !validate_schema(
                        telos_value_get(properties, name),
                        member,
                        depth + 1
                    )
                ) {
...
)
{
    ...
    descriptor = telos_registry_generation_find(
        generation,
    ...
    for (
        size_t index = 0;
        index < tool->required_capability_count;
        ++index
    ) {
        if (
            tool->required_capabilities == NULL
            || tool->required_capabilities[index] == NULL
            || !capability_available(
                broker,
                tool->required_capabilities[index]
            )
        ) {
            set_error(
                error,
    ...
    policy_request.required_capability_count =
        tool->required_capability_count;
    decision = broker->evaluate(
        &policy_request,
        broker->evaluate_context
    );
