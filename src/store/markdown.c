...
static char *copy_safe_text(const char *text)
{
    ...
    for (size_t index = 0; index < size; ++index) {
        ...
        copy[index] = character < 0x20 && character != '\0'
            ? ' '
            : (char)character;
        if (copy[index] == '`') {
            copy[index] = '\'';
...
static bool append_owned_event(
...
static bool value_integer_field(
...
static const char *value_string_field(
    const struct telos_value *object,
    const char *key
)
{
...
static struct telos_event *parse_envelope(
    const char *json,
    size_t size,
    struct telos_error **error
)
{
    ...
    if (
        envelope == NULL
        || telos_value_type(envelope) != TELOS_VALUE_OBJECT
        || !value_integer_field(envelope, "sequence", &sequence)
        || sequence <= 0
        || !value_integer_field(envelope, "timestamp_milliseconds", &timestamp)
        || (event_id = value_string_field(envelope, "event_id")) == NULL
        || (session_id = value_string_field(envelope, "session_id")) == NULL
        || (
        ...
        || (type = value_string_field(envelope, "type")) == NULL
        || (source = value_string_field(envelope, "source")) == NULL
        || (payload = telos_value_get(envelope, "payload")) == NULL
...
)
{
    ...
    if (
        fwrite(record, 1, record_size, markdown->file) != record_size
...
static struct telos_event *markdown_get(
...
)
{
    ...
    if (
        markdown->count == 0
        && fseek(markdown->file, 0, SEEK_END) == 0
