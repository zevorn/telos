...
)
{
    ...
    while (cursor != NULL && *cursor != '\0') {
        ...
        size_t directory_size = end == NULL
            ? strlen(cursor)
            : (size_t)(end - cursor);
...
)
{
    ...
    if (child == 0) {
        ...
        setenv(
            "PATH",
            host_path == NULL ? "/usr/bin:/bin" : host_path,
            1
        );
        ...
        if (
            environment != NULL
            && environment->pkgconfig_path != NULL
        ) {
            setenv(
                "PKG_CONFIG_PATH",
                environment->pkgconfig_path,
                1
            );
        }
        if (
            environment != NULL
            && environment->sdk_sysroot != NULL
        ) {
            setenv(
                "PKG_CONFIG_SYSROOT_DIR",
                environment->sdk_sysroot,
                1
            );
        }
        if (
            environment != NULL
            && environment->destination != NULL
        ) {
...
static bool permission_contains(
...
)
{
    ...
    for (
        size_t index = 0;
        index < telos_plugin_manifest_permission_count(manifest);
        ++index
    ) {
        const char *permission = telos_plugin_manifest_permission_at(
            manifest,
            index
        );
...
)
{
    struct telos_install_risk risk = {
        ...
        .filesystem_write = permission_contains(
            manifest,
            "filesystem.write"
        ),
    ...
    risk.requires_approval = risk.git_source
        || risk.native_build
        || risk.network
...
static bool write_build_inputs(
    const char *source_directory,
    const struct telos_install_options *options,
    char *path,
    size_t path_size,
...
static bool find_artifact_recursive(
...
static bool create_cache_entry(
    const struct telos_install_options *options,
    const struct telos_cancel *cancel,
    const char *quarantine,
    const char *source_directory,
...
)
{
    ...
    if (
        !path_join(entry, sizeof(entry), quarantine, "cache-entry")
...
)
{
    ...
    if (
        !path_join(plugins, sizeof(plugins), state_directory, "plugins")
        || !path_join(
            plugin_directory,
    ...
    if (options->builder == TELOS_BUILDER_CONTAINER) {
        ...
        if (
            ...
            || snprintf(
                ...
                "%s:/workspace:ro",
                source_directory
            ) >= (int)sizeof(source_mount)
            || snprintf(
                build_mount,
                sizeof(build_mount),
                "%s:/build:rw",
                build_directory
            ) >= (int)sizeof(build_mount)
            || snprintf(
                staging_mount,
                sizeof(staging_mount),
                "%s:/staging:rw",
                staging_directory
            ) >= (int)sizeof(staging_mount)
            || snprintf(
                user,
                sizeof(user),
                "%ld:%ld",
...
)
{
    ...
    if (
        !git_source
        && strncmp(options->source, "local:", 6) != 0
    ...
    if (
        ...
        || snprintf(
            ...
            "%s/install-XXXXXX",
            quarantine_root
        ) >= (int)sizeof(quarantine_template)
        || (quarantine = mkdtemp(quarantine_template)) == NULL
        || !path_join(
            source_directory,
...
)
{
    ...
    current_size = readlink(current, current_target, sizeof(current_target) - 1);
    previous_size = readlink(
        previous,
        previous_target,
        sizeof(previous_target) - 1
    );
