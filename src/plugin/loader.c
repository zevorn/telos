...
static bool registrar_add(void *context,
                          const struct telos_extension_descriptor *descriptor,
                          struct telos_error **error)
{
    struct registrar_context *registrar = context;

    return telos_registry_transaction_add(registrar->transaction, descriptor,
                                          error);
}
                                   ...
                                   struct telos_error **error)
{
    ...
    if (path == NULL || path[0] == '\0' || plugin_id == NULL ||
        plugin_id[0] == '\0' || host == NULL || registry == NULL) {
        set_error(error, TELOS_ERROR_DOMAIN_ARGUMENT, EINVAL,
                  "in-process Plugin load arguments are invalid");
        return NULL;
    }
    if (host->abi_version != TELOS_PLUGIN_ABI_VERSION ||
        host->struct_size <
            offsetof(struct telos_host_api_v1, log) + sizeof(host->log)) {
        set_error(error, TELOS_ERROR_DOMAIN_PLUGIN, EPROTONOSUPPORT,
    ...
    if (!telos_plugin_instance_transition(module->instance,
                                          TELOS_PLUGIN_VERIFIED, error)) {
        goto failure;
    }

    module->handle = dlopen(path, RTLD_NOW | RTLD_LOCAL);
    if (module->handle == NULL) {
        set_error(error, TELOS_ERROR_DOMAIN_PLUGIN, ENOENT, dlerror());
        goto failure;
    }
    if (!telos_plugin_instance_transition(module->instance, TELOS_PLUGIN_LOADED,
                                          error)) {
        goto failure;
    }
    ...
    if (symbol == NULL) {
        set_error(error, TELOS_ERROR_DOMAIN_PLUGIN, ENOSYS,
    ...
    transaction = telos_registry_transaction_begin(registry, plugin_id, error);
    if (transaction == NULL) {
        goto failure;
    }
    registrar_context.transaction = transaction;
    if (!telos_plugin_instance_transition(module->instance,
                                          TELOS_PLUGIN_INITIALIZING, error) ||
    ...
    if (!telos_plugin_instance_set_healthy(module->instance, true, error) ||
        !telos_plugin_instance_transition(module->instance, TELOS_PLUGIN_ACTIVE,
                                          error) ||
...
void telos_plugin_module_destroy(struct telos_plugin_module *module)
{
    ...
    if (telos_plugin_instance_state(module->instance) == TELOS_PLUGIN_ACTIVE) {
        can_unload = telos_plugin_instance_transition(
            module->instance, TELOS_PLUGIN_QUIESCING, NULL);
    }
    if (can_unload && telos_plugin_instance_state(module->instance) ==
                          TELOS_PLUGIN_QUIESCING) {
        can_unload = telos_plugin_instance_transition(
            module->instance, TELOS_PLUGIN_STOPPED, NULL);
    }
    if (can_unload &&
        telos_plugin_instance_state(module->instance) == TELOS_PLUGIN_STOPPED) {
        can_unload = telos_plugin_instance_transition(
            module->instance, TELOS_PLUGIN_UNLOADED, NULL);
    }
...
const struct telos_plugin_instance *
telos_plugin_module_instance(const struct telos_plugin_module *module)
{
