...
)
{
    ...
    if (
        ...
        || fragment->accepted_event_type_count == 0
    ...
    for (
        size_t index = 0;
        index < fragment->accepted_event_type_count;
        ++index
    ) {
        if (
            fragment->accepted_event_types[index] == NULL
...
)
{
    ...
    if (
        !telos_state_fragment_validate(fragment, error)
        || context == NULL
        || event == NULL
        || result == NULL
        || slot != fragment->slot
    ...
    for (
        ...
        if (
            strcmp(
                fragment->accepted_event_types[index],
                telos_event_type(event)
            ) == 0
        ) {
            accepted = true;
            break;
    ...
    *result = fragment->handle(context, event, error);
    if (
        *result < TELOS_FRAGMENT_COMPLETED
