...
struct telos_session_machine *
telos_session_machine_create(struct telos_error **error)
{
...
telos_session_machine_state(const struct telos_session_machine *machine)
{
    return machine == NULL ? 0 : machine->state;
}

unsigned int
telos_session_machine_retry_count(const struct telos_session_machine *machine)
{
    return machine == NULL ? 0 : machine->retry_count;
}

bool telos_session_machine_apply(struct telos_session_machine *machine,
                                 ...
                                 struct telos_error **error)
{
    ...
    if (sequence <= machine->last_sequence) {
        set_error(error, TELOS_ERROR_DOMAIN_STATE, EALREADY,
                  "session event sequence is stale");
        return false;
    }

    event_type = telos_event_type(event);
    if (machine->state == TELOS_SESSION_TOOL_AUTHORIZE &&
        strcmp(event_type, "tool.execute") == 0) {
        int64_t tool_count = 0;

        if (telos_value_integer(telos_event_payload(event), &tool_count) &&
            tool_count > 0) {
    ...
    if (machine->state == TELOS_SESSION_TOOL_EXECUTE &&
    ...
    if (machine->state == TELOS_SESSION_TOOL_EXECUTE &&
        strcmp(event_type, "tool.collect") == 0 && machine->pending_tools > 0) {
        set_error(error, TELOS_ERROR_DOMAIN_STATE, EBUSY,
                  "Tool Calls are still running");
        return false;
    }

    if (strcmp(event_type, "cancel.requested") == 0 &&
        machine->state != TELOS_SESSION_IDLE &&
    ...
    if (strcmp(event_type, "error.retryable") == 0 &&
        machine->state != TELOS_SESSION_IDLE &&
        machine->state != TELOS_SESSION_COMPLETED &&
        machine->state != TELOS_SESSION_CANCELLED &&
    ...
    if ((strcmp(event_type, "error.fatal") == 0 ||
    ...
    if (strcmp(event_type, "retry.resume") == 0 &&
        machine->state == TELOS_SESSION_RETRYING && machine->retry_state != 0) {
    ...
    for (size_t index = 0; index < sizeof(transitions) / sizeof(transitions[0]);
         ++index) {
        if (transitions[index].from == machine->state &&
