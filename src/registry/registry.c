...
static void entry_clear(struct registry_entry *entry)
{
    for (
        size_t index = 0;
        index < entry->descriptor.required_capability_count;
        ++index
    ) {
...
static bool entry_copy(
    struct registry_entry *target,
...
)
{
    ...
    if (source->required_capability_count > 0) {
        if (
            source->required_capabilities == NULL
        ...
        for (
            size_t index = 0;
            index < source->required_capability_count;
            ++index
        ) {
            target->capabilities[index] = copy_string(
                source->required_capabilities[index]
            );
...
static struct telos_registry_generation *generation_create(
    uint64_t number,
    size_t count
)
{
...
)
{
    ...
    if (capability_count > 0) {
        registry->capabilities = calloc(
            capability_count,
            sizeof(*registry->capabilities)
        );
...
)
{
    ...
    if (transaction->count == transaction->capacity) {
        capacity = transaction->capacity == 0
            ? 4
    ...
    if (!entry_copy(
        &transaction->entries[transaction->count],
        descriptor,
...
static bool registry_has_capability(
    const struct telos_registry *registry,
...
)
{
    ...
    for (
        ...
        if (
            descriptor->required_capabilities[index] == NULL
...
static bool descriptor_conflicts(
    const struct registry_entry *entries,
    size_t count,
...
)
{
    for (size_t index = 0; index < count; ++index) {
        if (
            entries[index].descriptor.kind == candidate->descriptor.kind
            && strcmp(
                entries[index].descriptor.id,
...
)
{
    ...
    for (size_t index = 0; index < generation->count; ++index) {
        if (
            strcmp(
                generation->entries[index].descriptor.plugin_id,
                plugin_id
            ) != 0
        ) {
            count += 1;
...
)
{
    ...
    for (size_t index = 0; index < transaction->count; ++index) {
        if (
            !descriptor_valid(registry, &transaction->entries[index])
        ...
        for (size_t prior = 0; prior < current->count; ++prior) {
            if (
                strcmp(
                    current->entries[prior].descriptor.plugin_id,
                    transaction->plugin_id
                ) != 0
                && current->entries[prior].descriptor.kind
                    == transaction->entries[index].descriptor.kind
                && strcmp(
                    current->entries[prior].descriptor.id,
                    transaction->entries[index].descriptor.id
                ) == 0
            ) {
                pthread_mutex_unlock(&registry->mutex);
                set_error(
    ...
    next = generation_create(
        current->number + 1,
        survivors + transaction->count
    );
    if (next == NULL) {
        pthread_mutex_unlock(&registry->mutex);
        set_error(
