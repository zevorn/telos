...
static bool write_all(int descriptor,
                      const void *data,
                      size_t size,
                      struct telos_error **error)
{
    ...
    while (size > 0) {
        ...
        if (written <= 0) {
            set_error(error, TELOS_ERROR_DOMAIN_IO, errno == 0 ? EPIPE : errno,
...
static bool
read_all(int descriptor, void *data, size_t size, struct telos_error **error)
{
    ...
    while (size > 0) {
        ...
        if (received <= 0) {
            set_error(error,
                      received == 0 ? TELOS_ERROR_DOMAIN_PROTOCOL
...
static bool message_valid(const struct telos_value *message)
{
    ...
    return message != NULL && telos_value_type(message) == TELOS_VALUE_OBJECT &&
           telos_value_integer(telos_value_get(message, "version"), &version) &&
           version == TELOS_RPC_VERSION &&
           (type = telos_value_string(telos_value_get(message, "type"))) !=
               NULL &&
           type[0] != '\0';
}

bool telos_rpc_write_frame(int file_descriptor,
                           ...
                           struct telos_error **error)
{
    ...
    if (json_size == 0 || json_size - 1 > TELOS_RPC_MAX_FRAME_SIZE ||
    ...
    result = write_all(file_descriptor, length, sizeof(length), error) &&
             write_all(file_descriptor, json, payload_size, error);
    free(json);
    return result;
}

struct telos_value *telos_rpc_read_frame(int file_descriptor,
                                         size_t maximum_size,
                                         struct telos_error **error)
{
    unsigned char length[4];
    ...
    payload_size = ((uint32_t)length[0] << 24) | ((uint32_t)length[1] << 16) |
