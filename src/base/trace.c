...
static struct telos_value *event_trace_value(const struct telos_event *event)
{
    ...
    const char *keys[] = {
        "sequence",       "event_id",     "session_id",
        "correlation_id", "causation_id", "type",
        "source",         "timestamp_ms", "payload",
    };
    ...
    values[0] = telos_value_new_integer((int64_t)telos_event_sequence(event));
    ...
    if (trace == NULL) {
        if (error != NULL) {
            *error = telos_error_create(
                event == NULL ? TELOS_ERROR_DOMAIN_ARGUMENT
                              : TELOS_ERROR_DOMAIN_MEMORY,
                event == NULL ? EINVAL : ENOMEM,
                "Event trace could not be constructed", NULL);
        }
        return false;
    }
    result = telos_value_write_json(trace, buffer, buffer_size, written, error);
    telos_value_release(trace);
    return result;
}
