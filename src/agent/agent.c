...
static bool add_call(struct response_collector *collector,
                     const struct telos_provider_event *event)
{
    ...
    if (event->call_id == NULL || event->call_id[0] == '\0' ||
        event->name == NULL || event->name[0] == '\0' ||
        event->payload == NULL) {
        return false;
    }
    for (size_t index = 0; index < collector->call_count; ++index) {
    ...
    if (collector->call_count == collector->call_capacity) {
        capacity =
            collector->call_capacity == 0 ? 4 : collector->call_capacity * 2;
        if (capacity < collector->call_capacity ||
        ...
        memset(calls + collector->call_capacity, 0,
...
static bool collect_event(const struct telos_provider_event *event,
                          void *context,
                          struct telos_error **error)
{
    ...
    case TELOS_PROVIDER_RESPONSE_STARTED:
        if (collector->response_id != NULL || event->response_id == NULL ||
    ...
    case TELOS_PROVIDER_ERROR:
        set_error(error, TELOS_ERROR_DOMAIN_PROTOCOL, EIO,
...
static void *execute_tool(void *context)
{
    ...
    if (!telos_tool_execute(worker->options->registry_generation,
                            worker->options->capability_broker,
                            TELOS_EXECUTION_CORE, worker->call->name,
                            worker->call->arguments, worker->cancel,
                            &worker->call->result, &worker->call->error) &&
                      ...
                      struct telos_error **error)
{
    ...
    for (; started < collector->call_count; ++started) {
        ...
        if (pthread_create(&threads[started], NULL, execute_tool,
                           &workers[started]) != 0) {
            break;
        }
    }
...
static struct telos_value *function_call_item(const struct pending_call *call)
{
    ...
    if (arguments_json != NULL && type != NULL && call_id != NULL &&
...
static struct telos_value *function_output_item(const struct pending_call *call)
{
    char *output_json = value_json(call->result);
    struct telos_value *type = telos_value_new_string("function_call_output");
    struct telos_value *call_id = telos_value_new_string(call->call_id);
    ...
    if (output_json != NULL && type != NULL && call_id != NULL &&
        output != NULL) {
...
continuation_items(const struct telos_value *prior_items,
                   ...
                   const struct response_collector *collector)
{
    ...
    size_t prior_count = state_mode == TELOS_PROVIDER_STATE_LOCAL
                             ...
                             : 0;
    size_t call_item_count =
        state_mode == TELOS_PROVIDER_STATE_LOCAL ? collector->call_count : 0;
    size_t item_count;

    if (prior_count > SIZE_MAX - call_item_count ||
        prior_count + call_item_count > SIZE_MAX - collector->call_count) {
        return NULL;
    }
    ...
    for (size_t index = 0; index < prior_count; ++index) {
        items[index] = telos_value_retain(telos_value_at(prior_items, index));
    }
    for (size_t index = 0; index < collector->call_count; ++index) {
        if (state_mode == TELOS_PROVIDER_STATE_LOCAL) {
            items[prior_count + index] =
                function_call_item(&collector->calls[index]);
        }
    ...
    result = telos_value_new_array((const struct telos_value *const *)items,
                                   item_count);
                     ...
                     struct telos_error **error)
{
    ...
    if (options == NULL || request == NULL || result == NULL ||
        options->registry_generation == NULL ||
        options->capability_broker == NULL || options->dispatch == NULL ||
        request->items == NULL ||
    ...
    for (size_t round = 0; round < maximum_rounds; ++round) {
        ...
        if (collector.call_count == 0) {
            result->text =
                collector.text == NULL ? copy_text("") : collector.text;
            collector.text = NULL;
            if (result->text == NULL) {
                set_error(error, TELOS_ERROR_DOMAIN_MEMORY, ENOMEM,
        ...
        if (current.state_mode == TELOS_PROVIDER_STATE_REMOTE &&
            ...
            set_error(
                error, TELOS_ERROR_DOMAIN_PROTOCOL, EPROTO,
        ...
        result->tool_calls += collector.call_count;
        {
            struct telos_value *next_items = continuation_items(
                current.items, current.state_mode, &collector);
        ...
        if (current.state_mode == TELOS_PROVIDER_STATE_REMOTE) {
            ...
            if (owned_previous_response_id == NULL) {
                set_error(
                    error, TELOS_ERROR_DOMAIN_MEMORY, ENOMEM,
    ...
    set_error(error, TELOS_ERROR_DOMAIN_STATE, ELOOP,
