...
The curl Transport is the official host HTTP transport for Telos Provider
Plugins on Linux and macOS. It streams response bytes directly to the Provider
callback and supports cooperative cancellation without buffering the complete
response.
...
`-Dcurl_transport=enabled` when building the interactive host Agent.
