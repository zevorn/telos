...
_Static_assert(sizeof(struct ring_store) <= TELOS_RING_STORE_STORAGE_SIZE,
               "TELOS_RING_STORE_STORAGE_SIZE is too small");
...
static bool ring_append(struct telos_event_store *store,
...
static struct telos_event *ring_get(const struct telos_event_store *store,
                                    size_t index, struct telos_error **error)
{
    const struct ring_store *ring = (const struct ring_store *)store;
    size_t physical;
                                 ...
                                 struct telos_error **error)
{
    ...
    if (storage == NULL || event_slots == NULL || capacity == 0 ||
        store == NULL || capacity > SIZE_MAX / sizeof(*event_slots)) {
        telos_store_set_error(error, TELOS_ERROR_DOMAIN_ARGUMENT, EINVAL,
                              "Ring Store static storage is invalid");
        return false;
    }

    memset(storage->bytes, 0, sizeof(storage->bytes));
    memset(event_slots, 0, capacity * sizeof(*event_slots));
    ring = (struct ring_store *)storage->bytes;
    ring->base.ops = &ring_ops;
    ring->capacity = capacity;
    ring->events = event_slots;
    *store = &ring->base;
    return true;
}

struct telos_event_store *telos_ring_store_create(size_t capacity,
                                                  struct telos_error **error)
{
    struct ring_store *ring;
