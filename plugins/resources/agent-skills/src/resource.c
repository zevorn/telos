...
#include <telos/plugins/agent_skills.h>
...
static char *read_file(const char *path, size_t maximum_size,
...
static char *frontmatter_value(const char *frontmatter, size_t size,
                               const char *key)
{
    ...
    while (cursor < end) {
        ...
        if ((size_t)(line_end - cursor) > key_size &&
            memcmp(cursor, key, key_size) == 0 && cursor[key_size] == ':') {
...
static bool load_skill(const char *directory, struct telos_skill *skill,
                       struct telos_error **error)
{
    ...
    skill->name =
        frontmatter_value(content + 4,
                          (size_t)(frontmatter_end - (content + 4)), "name");
    skill->description =
        frontmatter_value(content + 4,
    ...
    free(content);
    if (skill->name == NULL || skill->description == NULL ||
        skill->instructions == NULL || skill->directory == NULL) {
        skill_clear(skill);
        set_error(error, TELOS_ERROR_DOMAIN_PROTOCOL, EPROTO,
                  "SKILL.md requires name, description, and readable "
                  "instructions");
        return false;
    }
...
static bool list_directories(const char *root, char ***names, size_t *count,
                             struct telos_error **error)
{
    ...
    *count = 0;
    ...
    while ((entry = readdir(directory)) != NULL) {
        ...
        if (*count == capacity) {
            ...
            new_names = realloc(*names, new_capacity * sizeof(*new_names));
            if (new_names == NULL) {
                closedir(directory);
                free_names(*names, *count);
                set_error(error, TELOS_ERROR_DOMAIN_MEMORY, ENOMEM,
...
static bool append_skill(struct telos_resource_generation *generation,
                         const char *directory, struct telos_error **error)
{
    struct telos_skill *skills;

    if (generation->skill_count == SIZE_MAX ||
        generation->skill_count + 1 > SIZE_MAX / sizeof(*generation->skills)) {
        set_error(error, TELOS_ERROR_DOMAIN_MEMORY, ENOMEM,
                  "Skill Generation size overflow");
        return false;
    }
    skills = realloc(generation->skills,
    ...
    if (!load_skill(directory, &generation->skills[generation->skill_count],
                    error)) {
        return false;
    }
...
static struct telos_resource_generation *
scan_generation(struct telos_resource_manager *manager, uint64_t number,
                struct telos_error **error)
{
    ...
    for (size_t root_index = 0; root_index < manager->root_count;
         ++root_index) {
        char **names;
        size_t count;

        if (!list_directories(manager->roots[root_index], &names, &count,
                              error)) {
        ...
        for (size_t index = 0; index < count; ++index) {
            char *directory =
                join_path(manager->roots[root_index], names[index]);
            char *skill_path =
                directory == NULL ? NULL : join_path(directory, "SKILL.md");
...
telos_resource_manager_create(const char *const *skill_roots,
                              ...
                              struct telos_error **error)
{
    telos_resource_manager *manager;
    int result;
...
bool telos_resource_manager_reload(telos_resource_manager *manager,
                                   struct telos_error **error)
{
    telos_resource_generation *next;
    telos_resource_generation *prior;
    uint64_t number;
...
{
    telos_resource_generation *generation;
...
{
    telos_resource_generation *mutable_generation =
        (telos_resource_generation *)generation;
...
{
    return gen == NULL ? 0 : gen->skill_count;
}
...
telos_resource_generation_skill_at(const telos_resource_generation *gen,
                                   size_t index)
{
    if (gen == NULL || index >= gen->skill_count) {
        return NULL;
    }
    return &gen->skills[index];
}
...
static bool contains_case_insensitive(const char *haystack, const char *needle)
{
    ...
    for (; *haystack != '\0'; ++haystack) {
        ...
        while (index < needle_size && haystack[index] != '\0' &&
                                       ...
                                       struct telos_error **error)
{
    ...
    if (gen == NULL || request == NULL || request[0] == '\0') {
        set_error(error, TELOS_ERROR_DOMAIN_ARGUMENT, EINVAL,
    ...
    if (request[0] == '$') {
        ...
        for (size_t index = 0; index < gen->skill_count; ++index) {
            const struct telos_skill *skill = &gen->skills[index];

            if (strlen(skill->name) == (size_t)(end - request - 1) &&
                memcmp(skill->name, request + 1, (size_t)(end - request - 1)) ==
                    0) {
                return skill;
            }
        }
        ...
        for (size_t index = 0; index < gen->skill_count; ++index) {
            ...
            if (contains_case_insensitive(request, skill->name) ||
                contains_case_insensitive(request, skill->description)) {
                return skill;
            }
        }
    }
...
char *telos_skill_resolve_path(const struct telos_skill *skill,
                               ...
                               struct telos_error **error)
{
    ...
    if (strncmp(resolved, skill->directory, directory_size) != 0 ||
...
char *telos_skill_resolve_script(const struct telos_skill *skill,
                                 const char *relative_path,
                                 ...
                                 struct telos_error **error)
{
    ...
    for (size_t index = 0; index < available_capability_count; ++index) {
        if (available_capabilities[index] != NULL &&
            strcmp(available_capabilities[index], "process.spawn") == 0) {
            can_spawn = true;
            break;
        }
    }
    if (!can_spawn || relative_path == NULL ||
...
void telos_resource_string_free(char *value) { free(value); }
