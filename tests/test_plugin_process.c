...
int main(int argc, char **argv)
{
    ...
    telos_error_release(error);
    error = NULL;
    assert(telos_plugin_process_spawn_plugin(
        argv[1],
        NULL,
        "plugin",
        &error
    ...
    assert(telos_plugin_process_execute_tool(
        process,
    ...
    assert(strcmp(
        telos_value_string(response),
        "through loaded plugin"
    ) == 0);
    telos_value_release(response);
    response = NULL;
    assert(!telos_plugin_process_execute_tool(
...

    {
        ...
        for (
            size_t index = 0;
            index
                < sizeof(invalid_responses) / sizeof(invalid_responses[0]);
            ++index
        ) {
            process = telos_plugin_process_spawn(argv[1], &error);
            assert(process != NULL);
