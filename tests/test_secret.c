...
int main(void)
{
    ...
    clear_error(&error);
    assert(telos_secret_broker_resolve(
        NULL,
        reference,
        "provider.openai",
        capabilities,
    ...
    clear_error(&error);
    {
        struct telos_secret_broker *missing = telos_secret_broker_create(
            resolve_missing,
            NULL,
            NULL
        );
        struct telos_secret_broker *failing = telos_secret_broker_create(
            resolve_error,
            NULL,
            NULL
        );
