...
static bool write_text(const char *path, const char *content)
{
    ...
    bool result = file != NULL
        && fwrite(content, 1, size, file) == size;
...
)
{
    ...
    bool rejected = !telos_plugin_install(
        options,
        cancel,
        &result,
        &error
...
{
    struct telos_error *error = NULL;
    bool rejected = !telos_plugin_activate(state, plugin_id, key, &error)
        && error != NULL;
...
{
    struct telos_error *error = NULL;
    bool rejected = !telos_plugin_remove(state, plugin_id, &error)
        && error != NULL;
...
{
    struct telos_error *error = NULL;
    bool rejected = !telos_plugin_rollback(state, plugin_id, &error)
        && error != NULL;
...
int main(void)
{
    ...
    passed = passed
        && activate_rejected(NULL, "plugin", KEY_ZERO)
    ...
    passed = passed
        ...
        && make_directory(cache_plugins);
    passed = passed
        && make_directory(zero)
        && make_directory(zero_staged)
        && make_directory(zero_nested)
        && write_text(zero_artifact, "fixture")
        && make_directory(one)
        ...
        && error == NULL
        && telos_plugin_rollback(state, "plugin", &error)
        && error == NULL;

    passed = passed
        && rollback_rejected(NULL, "plugin")
        && rollback_rejected(state, NULL)
        && rollback_rejected(state, "bad/id")
        && rollback_rejected(state, "missing")
        && remove_rejected(NULL, "plugin")
        && remove_rejected(state, NULL)
        && remove_rejected(state, "")
        && remove_rejected(state, "bad/id")
        && remove_rejected(state, "missing");

    passed = passed
        && make_directory(removable)
        && make_directory(removable_nested)
        && write_text(removable_file, "fixture")
        && symlink(removable_file, removable_link) == 0
        && telos_plugin_remove(state, "remove-me", &error)
        && error == NULL
        && access(removable, F_OK) != 0
        && telos_plugin_remove(state, "plugin", &error)
        && error == NULL;
