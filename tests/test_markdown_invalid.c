...
{
    size_t size = strlen(HEADER) + strlen(FENCE) + strlen(json)
        + strlen(END) + 32;
...
static struct telos_event *new_event(
    uint64_t sequence,
    const char *type,
    const char *source
)
...
static bool recovery_matrix(void)
{
    ...
    store = telos_markdown_store_create(path, NULL);
    {
        ...
        passed = passed
            && store != NULL
            && telos_event_store_count(store) == 9
            && last != NULL
...
static bool append_matrix(void)
{
    ...
    passed = store != NULL
        ...
        && duplicate != NULL
        && overflow != NULL
        && telos_event_store_append(store, safe, NULL)
        && !telos_event_store_append(store, duplicate, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && !telos_event_store_append(store, overflow, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && telos_event_store_get(store, 1, &error) == NULL
        && error != NULL;
    telos_error_release(error);
...
int main(void)
{
    ...
    passed = passed
        && rejects_content("wrong header\n")
        && rejects_content(HEADER "x")
        && rejects_content(HEADER "## record")
        && rejects_content(HEADER "## record\n" FENCE "{}")
        && rejects_json("{invalid}")
        && rejects_json("[]")
        && rejects_json("{}")
