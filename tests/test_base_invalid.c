...
int main(void)
{
    ...
    assert(telos_error_create(
        TELOS_ERROR_DOMAIN_PLUGIN + 1,
        1,
        "bad",
        NULL
    ) == NULL);
    ...
    cause = telos_error_create(
        TELOS_ERROR_DOMAIN_IO,
        EIO,
        "cause",
        NULL
    );
    outer = telos_error_create(
    ...
    assert(!telos_id_parse(
        "gggggggggggggggg0000000000000000",
        &parsed
    ));
    assert(!telos_id_parse(
        "0000000000000000gggggggggggggggg",
        &parsed
    ));
    assert(!telos_id_parse(
    ...
    assert(!telos_event_write_trace_json(
        NULL,
        buffer,
        sizeof(buffer),
        NULL,
