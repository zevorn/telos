...
int main(void)
{
    ...
    if (
        ...
        || mkdir(nested, 0700) != 0
        || !write_text(user_file, "user guidance")
    ...
    user = NULL;
    passed = passed
        && write_text(nested_file, "updated nested guidance")
        && telos_guidance_discover(
            home,
            project,
            nested,
    ...
    passed = passed
        && second != NULL
        && strstr(
            telos_prompt_snapshot_content(first),
            "nested guidance"
        ) != NULL
        && strstr(
            telos_prompt_snapshot_content(first),
            "updated nested guidance"
        ) == NULL
        && strstr(
            telos_prompt_snapshot_content(second),
            "updated nested guidance"
        ) != NULL;
