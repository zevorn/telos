...
)
{
    ...
    result = fprintf(
        ...
        fields->modes,
        fields->default_mode,
        fields->runtime_extra,
        fields->targets,
        fields->platform_extra,
        fields->system,
        fields->build_extra,
        fields->permissions,
        fields->permission_extra
    ) > 0;
    return fclose(file) == 0 && result;
}

static bool manifest_rejected(
    const char *path,
...
static bool verify_path_rejected(
...
static bool manifest_matrix(const char *path)
{
    ...
    for (
        ...
        passed
            && index
                < sizeof(invalid_permissions) / sizeof(invalid_permissions[0]);
        ++index
    ) {
        fields = valid_fields;
    ...
    passed = passed
        && content_rejected(path, "id = \"dev.example.bad\"\n")
    ...
    passed = passed
        ...
        && telos_plugin_manifest_runtime_modes(manifest)
            == (
                ...
                | TELOS_PLUGIN_RUNTIME_INPROCESS
...
)
{
    ...
    for (
        size_t index = 0;
        passed
            && index < sizeof(invalid_locks) / sizeof(invalid_locks[0]);
        ++index
    ) {
    ...
    passed = passed
        && !telos_plugin_lock_verify(NULL, directory, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && !telos_plugin_source_digest(NULL, digest, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && !telos_plugin_source_digest(directory, NULL, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
    ...
    error = NULL;
    passed = passed
        && !telos_plugin_lock_write(NULL, lock_path, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && !telos_plugin_lock_write(lock, "", &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && !telos_plugin_lock_write(lock, directory, &error)
        && error != NULL;
    telos_error_release(error);
    telos_plugin_lock_destroy(lock);
    telos_plugin_lock_destroy(NULL);
    return passed
        && telos_plugin_lock_source_hash(NULL) == NULL
...
int main(void)
{
    ...
    passed = write_text(outside_path, "outside")
        ...
        && telos_plugin_manifest_load(NULL, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && telos_plugin_manifest_load("", &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
