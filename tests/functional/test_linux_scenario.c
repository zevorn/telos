...
static struct telos_value *tool_schema(void)
{
    ...
    struct telos_value *text_schema =
        telos_value_new_object(string_keys, string_values, 1);
    const char *property_keys[] = {"text"};
    const struct telos_value *property_values[] = {text_schema};
    struct telos_value *properties =
        telos_value_new_object(property_keys, property_values, 1);
...
static bool process_echo(const struct telos_tool_context *context,
                         const struct telos_value *arguments,
                         struct telos_value **result,
                         struct telos_error **error)
{
    return telos_plugin_process_execute_tool(
        tool_process, "dev.zevorn.process-echo", arguments, 1000,
        context->cancel, result, error);
}

static enum telos_policy_decision
allow_echo(const struct telos_policy_request *request, void *context)
{
...
static bool provider_dispatch(const struct telos_provider_request *request,
                              ...
                              struct telos_error **error)
{
    ...
    if (provider->rounds == 1) {
        ...
        const char *output =
            telos_value_string(telos_value_get(item, "output"));
        struct telos_value *parsed =
            telos_value_parse_json(output, strlen(output), NULL);

        assert(telos_value_count(request->items) == 2);
        assert(strcmp(telos_value_string(telos_value_get(parsed, "text")),
                      "from-process") == 0);
...
event(uint64_t sequence, const char *type, const struct telos_value *payload)
{
...
int main(int argc, char **argv)
{
    ...
    assert(prompt != NULL);
    assert(strstr(telos_prompt_snapshot_content(prompt), "USER GUIDANCE") <
           strstr(telos_prompt_snapshot_content(prompt), "PROJECT GUIDANCE"));
    transaction = telos_registry_transaction_begin(
        registry, "dev.zevorn.process-fixture", NULL);
    ...
    broker = telos_capability_broker_create(NULL, 0, allow_echo,
                                            &authorizations, NULL);
