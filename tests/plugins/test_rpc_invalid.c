...
static struct telos_value *message(const struct telos_value *version,
...
static bool write_rejected(int descriptor, const struct telos_value *value)
{
    struct telos_error *error = NULL;
    bool rejected =
        !telos_rpc_write_frame(descriptor, value, &error) && error != NULL;
...
int main(void)
{
    ...
    large_body = large_text == NULL ? NULL : telos_value_new_string(large_text);
    large = large_body == NULL ? NULL : message(version, type, large_body);
    passed =
        version != NULL && type != NULL && body != NULL && valid != NULL &&
        large != NULL && write_rejected(-1, valid) &&
        ...
        rejects(partial_header, sizeof(partial_header),
        ...
        rejects(missing_version, sizeof(missing_version),
                TELOS_RPC_MAX_FRAME_SIZE) &&
        rejects(empty_type, sizeof(empty_type), TELOS_RPC_MAX_FRAME_SIZE) &&
        rejects(wrong_version, sizeof(wrong_version), TELOS_RPC_MAX_FRAME_SIZE);
    ...
    telos_error_release(error);
    error = NULL;
    passed = passed && telos_rpc_read_frame(STDIN_FILENO, 0, &error) == NULL &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    if (pipe(descriptors) != 0) {
        ...
        close(descriptors[0]);
        passed = telos_rpc_read_frame(descriptors[0], TELOS_RPC_MAX_FRAME_SIZE,
                                      &error) == NULL &&
                 error != NULL && passed;
        telos_error_release(error);
        error = NULL;
