...
static void assert_load_fails(const char *path,
                              const char *plugin_id,
                              const struct telos_host_api_v1 *host,
                              struct telos_registry *registry,
                              struct telos_error **error)
{
    assert(telos_plugin_module_load_inprocess(path, plugin_id, host, registry,
                                              error) == NULL);
    clear_error(error);
}
...
int main(int argc, char **argv)
{
    ...
    assert_load_fails(argv[1], "fixture", &short_host, registry, &error);
    assert_load_fails("/definitely/missing/telos-plugin.so", "fixture", &host,
                      registry, &error);
...

    {
        ...
        assert(telos_registry_transaction_add(transaction, &conflict, NULL));
        ...
        assert(telos_registry_generation_find(generation,
                                              TELOS_EXTENSION_PROVIDER,
                                              "fixture.responses") != NULL);
        assert(telos_registry_generation_find(generation, TELOS_EXTENSION_TOOL,
                                              "fixture.echo") == NULL);
        telos_registry_generation_release(generation);
    }
