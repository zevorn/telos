...
static struct telos_event *new_event(void)
{
    ...
    struct telos_value *sensitive =
        telos_value_new_sensitive("never-write-this-token");
...
int main(void)
{
    ...
    if (store == NULL || error != NULL ||
        !telos_event_store_append(store, event, &error) || error != NULL) {
    ...
    if (content == NULL || strstr(content, "## 42 · tool.completed") == NULL ||
    ...
    if (store == NULL || error != NULL || telos_event_store_count(store) != 1 ||
        recovered == NULL || telos_event_sequence(recovered) != 42 ||
        !telos_id_equal(telos_event_id(recovered), telos_event_id(event)) ||
        !telos_id_equal(telos_event_session_id(recovered),
                        telos_event_session_id(event)) ||
        strcmp(telos_event_type(recovered), "tool.completed") != 0 ||
        strcmp(telos_event_source(recovered), "plugin:com.example.echo") != 0) {
