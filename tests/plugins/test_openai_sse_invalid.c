...
static bool run(const char *stream,
                size_t size,
                bool finish,
                ...
                bool expected)
{
    struct telos_openai_sse_parser *parser =
        telos_openai_sse_parser_create(policy, callback, NULL, NULL);
    ...
    result = result == expected && (expected || error != NULL);
...
{
    return run(stream, size, finish, TELOS_OPENAI_UNKNOWN_EVENT_ERROR,
               accept_event, false);
}

static bool accepts(const char *stream)
{
    return run(stream, strlen(stream), true, TELOS_OPENAI_UNKNOWN_EVENT_ERROR,
               accept_event, true);
}

int main(void)
{
    static const char unknown[] = "event: response.future\n"
    ...
    static const char no_data[] = "event: response.failed\n\n";
    static const char invalid_json[] = "data: {invalid}\n\n";
    static const char scalar_json[] = "data: []\n\n";
    ...
    static const char valid_variants[] =
        ...
        "\"response\":{\"id\":\"response\"}}\n\n"
        "data: [DONE]\n\n";
    static const char multiline_data[] = "event: response.failed\n"
                                         "data: {\"type\":\n"
    ...
    bool passed =
        large != NULL &&
        run(unknown, strlen(unknown), true, TELOS_OPENAI_UNKNOWN_EVENT_IGNORE,
            accept_event, true) &&
        ...
        accepts(valid_variants) && accepts(multiline_data) &&
        accepts(valid_utf8) &&
        run("data: {\"type\":\"response.failed\"}\n\n",
            strlen("data: {\"type\":\"response.failed\"}\n\n"), true,
            TELOS_OPENAI_UNKNOWN_EVENT_ERROR, reject_event, false);

    passed =
        passed &&
        telos_openai_sse_parser_create(0, accept_event, NULL, &error) == NULL &&
        error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             telos_openai_sse_parser_create(TELOS_OPENAI_UNKNOWN_EVENT_ERROR,
    ...
    parser = telos_openai_sse_parser_create(TELOS_OPENAI_UNKNOWN_EVENT_ERROR,
                                            accept_event, NULL, NULL);
    passed = passed && !telos_openai_sse_parser_feed(NULL, "", 0, &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed && !telos_openai_sse_parser_feed(parser, NULL, 1, &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed && telos_openai_sse_parser_feed(parser, NULL, 0, NULL) &&
             telos_openai_sse_parser_feed(parser, " \r\n\t", 4, NULL) &&
             telos_openai_sse_parser_finish(parser, NULL) &&
             !telos_openai_sse_parser_finish(parser, &error) && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed && !telos_openai_sse_parser_feed(parser, "", 0, &error) &&
             error != NULL;
    telos_error_release(error);
    telos_openai_sse_parser_destroy(parser);
    ...
    if (large == NULL) {
        ...
        parser = telos_openai_sse_parser_create(
            TELOS_OPENAI_UNKNOWN_EVENT_ERROR, accept_event, NULL, NULL);
        passed = !telos_openai_sse_parser_feed(parser, large, 1024U * 1024U + 1,
                                               &error) &&
                 error != NULL && passed;
        telos_error_release(error);
        error = NULL;
        ...
        parser = telos_openai_sse_parser_create(
            TELOS_OPENAI_UNKNOWN_EVENT_ERROR, accept_event, NULL, NULL);
        passed =
            telos_openai_sse_parser_feed(parser, large, 600U * 1024U, NULL) &&
            !telos_openai_sse_parser_feed(parser, large, 500U * 1024U,
                                          &error) &&
            error != NULL && passed;
        telos_error_release(error);
        telos_openai_sse_parser_destroy(parser);
