...
int main(int argc, char **argv)
{
    ...
    assert(error == NULL);
    assert(strcmp(telos_plugin_manifest_id(manifest),
                  "dev.example.echo-tool") == 0);
    assert(telos_plugin_manifest_abi(manifest) == 1);
    assert(telos_plugin_manifest_default_runtime(manifest) ==
    ...
    snprintf(dependency_path, sizeof(dependency_path), "%s/dependency.txt",
             directory);
    snprintf(invalid_path, sizeof(invalid_path), "%s/invalid.toml", directory);
    snprintf(multiline_path, sizeof(multiline_path), "%s/multiline.toml",
             directory);
    snprintf(source_directory, sizeof(source_directory), "%s/source",
             directory);
    snprintf(source_lock_path, sizeof(source_lock_path), "%s/telos.lock",
             source_directory);
    snprintf(source_payload_path, sizeof(source_payload_path), "%s/payload.txt",
             source_directory);
    write_file(dependency_path, "hello\n");
    assert(mkdir(source_directory, 0700) == 0);
    write_file(source_payload_path, "source payload\n");
    assert(telos_plugin_source_digest(source_directory, source_digest, &error));
    {
        char source_lock[256];

        assert(
            snprintf(source_lock, sizeof(source_lock),
                     "format = 1\nsource_hash = \"%s\"\n\ndependencies = []\n",
                     source_digest) < (int)sizeof(source_lock));
        write_file(source_lock_path, source_lock);
    }
    lock = telos_plugin_lock_load(source_lock_path, &error);
    assert(lock != NULL);
    assert(telos_plugin_lock_verify_source(lock, source_directory, &error));
    ...
    write_file(multiline_path, "[plugin]\n"
    ...
    assert(strcmp(telos_plugin_manifest_permission_at(manifest, 1),
                  "network.http") == 0);
    telos_plugin_manifest_destroy(manifest);

    write_file(invalid_path, "[plugin]\n"
                             "id = \"Invalid ID\"\n"
                             "name = \"bad\"\n"
                             "version = \"1\"\n"
                             "abi = 9\n"
