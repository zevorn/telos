...
static void assert_reload_rejected(struct telos_resource_manager *manager,
                                   const char *path,
                                   const char *content,
...
int main(void)
{
    ...
    assert(snprintf(script_directory, sizeof(script_directory), "%s/scripts",
                    skill) < (int)sizeof(script_directory));
    assert(snprintf(script_path, sizeof(script_path), "%s/not-executable.sh",
                    script_directory) < (int)sizeof(script_path));
    assert(snprintf(escaped_path, sizeof(escaped_path), "%s/escaped", skill) <
           (int)sizeof(escaped_path));
    assert(snprintf(invalid_skill, sizeof(invalid_skill), "%s/invalid", root) <
           (int)sizeof(invalid_skill));
    assert(snprintf(invalid_path, sizeof(invalid_path), "%s/SKILL.md",
                    invalid_skill) < (int)sizeof(invalid_path));
    assert(snprintf(ignored_file, sizeof(ignored_file), "%s/not-a-directory",
                    root) < (int)sizeof(ignored_file));
    assert(snprintf(ignored_directory, sizeof(ignored_directory), "%s/no-skill",
                    root) < (int)sizeof(ignored_directory));
    assert(snprintf(regular_root, sizeof(regular_root), "%s/regular-root",
                    root) < (int)sizeof(regular_root));
    ...
    selected =
        telos_resource_generation_select_skill(generation, "$valid", &error);
    ...
    assert(telos_resource_generation_select_skill(
               generation, "$valid with arguments", &error) == selected);
    assert(telos_resource_generation_select_skill(
               generation, "Please use VALID now", &error) == selected);
    assert(telos_resource_generation_select_skill(
               generation, "This is A VALID FIXTURE", &error) == selected);
    assert(telos_resource_generation_select_skill(NULL, "$valid", &error) ==
           NULL);
    clear_error(&error);
    assert(telos_resource_generation_select_skill(generation, NULL, &error) ==
           NULL);
    clear_error(&error);
    assert(telos_resource_generation_select_skill(generation, "", &error) ==
           NULL);
    clear_error(&error);
    assert(telos_resource_generation_select_skill(generation, "no match",
                                                  &error) == NULL);
    ...
    assert(telos_error_domain(error) == TELOS_ERROR_DOMAIN_PERMISSION);
    clear_error(&error);
    resolved = telos_skill_resolve_script(selected, "scripts/not-executable.sh",
                                          capabilities, 1, &error);
    assert(resolved == NULL);
    assert(telos_error_domain(error) == TELOS_ERROR_DOMAIN_PERMISSION);
    clear_error(&error);
    assert(telos_skill_resolve_script(selected, "scripts/not-executable.sh",
                                      NULL, 1, &error) == NULL);
    assert(telos_error_domain(error) == TELOS_ERROR_DOMAIN_ARGUMENT);
    clear_error(&error);
    assert(telos_skill_resolve_script(selected, NULL, capabilities, 1,
                                      &error) == NULL);
    clear_error(&error);
    assert(telos_skill_resolve_script(selected, "SKILL.md", capabilities, 1,
                                      &error) == NULL);
    clear_error(&error);
    assert(telos_skill_resolve_script(selected, "scripts/not-executable.sh",
                                      unrelated, 2, &error) == NULL);
    clear_error(&error);
    assert(telos_skill_resolve_script(selected, "scripts/", capabilities, 1,
                                      &error) == NULL);
    clear_error(&error);

    assert(mkdir(invalid_skill, 0700) == 0);
    assert_reload_rejected(manager, invalid_path, "No frontmatter.\n", &error);
    assert_reload_rejected(manager, invalid_path,
                           "---\nname: invalid\nNo terminator.\n", &error);
    assert_reload_rejected(manager, invalid_path,
                           "---\nname:\ndescription: fixture\n---\nBody.\n",
                           &error);
