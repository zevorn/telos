...
static bool verify_store(struct telos_event_store *store)
{
    ...
    if (telos_event_store_count(store) != 2 ||
        telos_event_sequence(first) != 2 || telos_event_sequence(second) != 3) {
...
int main(void)
{
    union telos_ring_store_storage storage;
    struct telos_event *event_slots[2];
    struct telos_event_store *dynamic = telos_ring_store_create(2, NULL);
    struct telos_event_store *fixed = NULL;
    bool passed = dynamic != NULL && verify_store(dynamic);

    telos_event_store_destroy(dynamic);
    passed = passed &&
             telos_ring_store_initialize(&storage, event_slots,
                                         TELOS_ARRAY_SIZE(event_slots), &fixed,
                                         NULL) &&
             verify_store(fixed);
    telos_event_store_destroy(fixed);
    if (!passed) {
        return 1;
    }
    return 0;
