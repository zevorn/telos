                            ...
                            struct telos_error **error)
{
    ...
    if (strcmp(reference, "secret:provider.openai") != 0 ||
        strcmp(target, "provider.openai") != 0 || value == NULL ||
        value[0] == '\0') {
...
static size_t
receive_curl_data(char *data, size_t size, size_t count, void *context)
{
...
static bool send_https(const struct telos_transport_request *request,
                       telos_transport_chunk_fn receive,
                       ...
                       struct telos_error **error)
{
    ...
    (void)transport_context;
    if (curl == NULL || request == NULL || receive == NULL ||
        status_code == NULL) {
    ...
    next_header = curl_slist_append(headers, "Content-Type: application/json");
    if (next_header == NULL) {
...
{
    fputs("usage: telos-openai-smoke [PROMPT]\n"
          "\n"
...
int main(int argc, char **argv)
{
    ...
    secret_broker = telos_secret_broker_create(resolve_secret, NULL, &error);
    {
    ...
    if (secret_broker == NULL || provider == NULL || registry == NULL ||
        generation == NULL || capability_broker == NULL || role == NULL ||
        content == NULL || item == NULL || items == NULL || tools == NULL ||
    ...
    }
    {
        ...
        if (!telos_agent_run(&options, &request, NULL, &agent_result, &error)) {
            goto cleanup;
        }
    }
    puts(agent_result.text);
    fprintf(stderr,
            "Telos remote smoke completed: provider_rounds=%zu "
            "tool_calls=%zu\n",
            agent_result.provider_rounds, agent_result.tool_calls);
    exit_code = 0;

cleanup:
    if (exit_code != 0) {
        fprintf(stderr, "telos-openai-smoke: %s\n",
                error == NULL ? "initialization failed"
