                          ...
                          struct telos_error **error)
{
    ...
    if (event->kind == TELOS_PROVIDER_TOOL_CALL_COMPLETED) {
        const char *text =
            telos_value_string(telos_value_get(event->payload, "text"));

        capture->completed_arguments_valid =
            text != NULL && strcmp(text, "hello") == 0;
    }
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    struct telos_openai_sse_parser *parser = telos_openai_sse_parser_create(
        TELOS_OPENAI_UNKNOWN_EVENT_ERROR, capture_event, &capture, NULL);
    size_t size;
    char *fixture;
    bool passed;
    ...
    for (size_t offset = 0; passed && offset < size;) {
        ...
        passed =
            telos_openai_sse_parser_feed(parser, fixture + offset, chunk, NULL);
        offset += chunk;
    }
    passed = passed && telos_openai_sse_parser_finish(parser, NULL) &&
             capture.count == 10 &&
