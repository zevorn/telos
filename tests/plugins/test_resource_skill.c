...
int main(void)
{
    ...
    if (!join_path(skill_directory, sizeof(skill_directory), root, "deploy") ||
        !join_path(agents_directory, sizeof(agents_directory), skill_directory,
                   "agents") ||
        !join_path(skill_path, sizeof(skill_path), skill_directory,
                   "SKILL.md") ||
        !join_path(metadata_path, sizeof(metadata_path), agents_directory,
                   "openai.yaml") ||
        !join_path(reference_directory, sizeof(reference_directory),
                   skill_directory, "references") ||
        !join_path(reference_path, sizeof(reference_path), reference_directory,
                   "guide.md") ||
        !join_path(script_directory, sizeof(script_directory), skill_directory,
                   "scripts") ||
        !join_path(script_path, sizeof(script_path), script_directory,
                   "deploy.sh") ||
    ...
    selected = telos_resource_generation_select_skill(first, "$deploy", NULL);
    resolved = telos_skill_resolve_path(selected, "references/guide.md", NULL);
    passed = manager != NULL && first != NULL &&
             ...
             strstr(telos_skill_instructions(selected),
                    "Run the deployment checks.") != NULL &&
             telos_skill_has_openai_metadata(selected) && resolved != NULL;
    telos_resource_string_free(resolved);
    resolved = telos_skill_resolve_script(selected, "scripts/deploy.sh", NULL,
                                          0, NULL);
    passed = passed && resolved == NULL;
    resolved = telos_skill_resolve_script(selected, "scripts/deploy.sh",
                                          script_capabilities, 1, NULL);
    passed = passed && resolved != NULL;
    telos_resource_string_free(resolved);
    ...
    selected = telos_resource_generation_select_skill(second,
                                                      "please handle a "
                                                      "production deployment",
                                                      NULL);
    passed =
        passed && second != NULL &&
        telos_resource_generation_number(second) == 2 && selected != NULL &&
        strstr(telos_skill_instructions(selected), "new release workflow") !=
            NULL &&
        strstr(telos_skill_instructions(telos_resource_generation_select_skill(
                   first, "$deploy", NULL)),
               "deployment checks") != NULL &&
