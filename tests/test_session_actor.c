...
int main(void)
{
    ...
    struct telos_session_actor *first = telos_session_actor_create(
        &first_spec,
        NULL
    );
    struct telos_session_actor *second = telos_session_actor_create(
        &second_spec,
        NULL
    );
    ...
    if (passed) {
        ...
        for (
            size_t index = 0;
            index < sizeof(remaining) / sizeof(remaining[0]);
            ++index
        ) {
            struct telos_event *event = new_event(index + 2, remaining[index]);

            passed = event != NULL
                && telos_session_actor_submit(first, event, NULL);
