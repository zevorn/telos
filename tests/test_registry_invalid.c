...
static bool invalid_descriptor(
    struct telos_registry *registry,
...
)
{
    ...
    bool rejected = transaction != NULL
        && telos_registry_transaction_add(transaction, &descriptor, NULL)
...
int main(void)
{
    ...
    bool passed = telos_registry_create(NULL, 1, &error) == NULL
        && error != NULL;
    ...
    passed = passed
        ...
        && generation != NULL
        && retained == generation
        && telos_registry_acquire(NULL) == NULL
    ...
    error = NULL;
    passed = passed
        && !telos_registry_transaction_commit(NULL, &error)
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
    ...
    generation = telos_registry_acquire(registry);
    passed = passed
        && telos_registry_generation_number(generation) == 4
