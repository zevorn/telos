...
int main(void)
{
    ...
    struct telos_value *options = telos_value_new_object(
        option_keys,
        option_values,
        1
    );
    ...
    bool passed = local_json != NULL
        ...
        && !store
        && telos_value_boolean(
            telos_value_get(local_json, "stream"),
            &stream
        )
        && stream
        && telos_value_get(local_json, "options") == NULL
        && telos_value_real(
            telos_value_get(local_json, "temperature"),
            &mapped_temperature
        )
...

    {
        ...
        struct telos_value *invalid_options = telos_value_new_object(
            keys,
            values,
            1
        );
        ...
        invalid_json = telos_openai_responses_build_request(
            "fixture-model",
            &invalid,
            &error
        );
