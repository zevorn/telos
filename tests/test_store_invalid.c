...
static bool test_store(struct telos_event_store *store)
{
    ...
    for (uint64_t sequence = 1; passed && sequence <= 10; ++sequence) {
        ...
        passed = event != NULL
            && telos_event_store_append(store, event, NULL);
    ...
    }
    {
        ...
        error = NULL;
        passed = passed
            && !telos_event_store_append(store, older, &error)
            && error != NULL;
        telos_error_release(error);
        error = NULL;
    ...
    passed = passed
        && telos_event_store_get(store, telos_event_store_count(store), &error)
            == NULL
        && error != NULL;
    telos_error_release(error);
...
int main(void)
{
    ...
    error = NULL;
    passed = passed
        && telos_event_store_get(NULL, 0, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    telos_error_release(error);
    error = NULL;
    invalid = telos_markdown_store_create(
        "/missing/telos/store/events.md",
        &error
    );
    passed = passed && invalid == NULL && error != NULL;
    telos_error_release(error);
    ...
    passed = passed
        && test_store(telos_memory_store_create(NULL))
