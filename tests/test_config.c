...
int main(void)
{
    ...
    assert(
        telos_config_get_origin(config, "agent.provider")
        == TELOS_CONFIG_ENVIRONMENT
    );
    assert(strcmp(
        telos_config_get(config, "agent.model"),
        "user-model"
    ) == 0);
    assert(
        telos_config_get_origin(config, "agent.model")
        == TELOS_CONFIG_USER
    );
    assert(strcmp(
        telos_config_get(config, "builder.backend"),
        "native"
    ) == 0);
    assert(telos_config_override(
        config,
        "agent.provider",
        "cli-provider",
        &error
