...
static void exercise_allocation_failures(operation_fn operation, void *context)
{
...
static bool json_operation(void *context)
{
    ...
    result = value != NULL && telos_value_write_json(
                                  value, output, sizeof(output), NULL, &error);
...
static bool error_operation(void *context)
{
    struct telos_error *cause =
        telos_error_create(TELOS_ERROR_DOMAIN_IO, 1, "cause", NULL);
    struct telos_error *outer =
        cause == NULL
            ? NULL
            : telos_error_create(TELOS_ERROR_DOMAIN_STATE, 2, context, cause);
...
static bool prompt_operation(void *context)
{
    ...
    struct telos_prompt_snapshot *snapshot =
        telos_prompt_snapshot_create(fragments, 2, &error);
...
static char *resolve_secret(const char *reference,
                            const char *target,
                            void *context,
...
static bool secret_operation(void *context)
{
    ...
    struct telos_secret_broker *broker =
        reference == NULL
            ? NULL
            : telos_secret_broker_create(resolve_secret, context, &error);
    struct telos_secret_material *material =
        broker == NULL
            ? NULL
            : telos_secret_broker_resolve(broker, reference, "provider.openai",
...
static bool registry_operation(void *context)
{
    ...
    bool result =
        transaction != NULL &&
        telos_registry_transaction_add(transaction, &descriptor, &error) &&
...
static bool plugin_operation(void *context)
{
    ...
    struct telos_plugin_instance *instance =
        telos_plugin_instance_create(context, &error);
...
static bool store_operation(void *context)
{
    ...
    for (uint64_t sequence = 1; result && sequence <= 12; ++sequence) {
        ...
        struct telos_event *event =
            payload == NULL ? NULL : telos_event_create(&spec, &error);

        result =
            event != NULL && telos_event_store_append(store, event, &error);
...
static bool markdown_store_operation(void *context)
{
    ...
    result = event != NULL && telos_event_store_append(store, event, &error);
    telos_event_release(event);
    telos_value_release(payload);
    telos_event_store_destroy(store);
    telos_error_release(error);
...
static bool trace_operation(void *context)
{
    ...
    result = size > 0 && size <= sizeof(output) &&
             telos_event_write_trace_json(event, output, sizeof(output), NULL,
                                          &error);
    telos_event_release(event);
    telos_value_release(payload);
    telos_error_release(error);
...
static bool manifest_operation(void *context)
{
    ...
    struct telos_plugin_manifest *manifest =
        telos_plugin_manifest_load(paths->manifest, &error);
    struct telos_plugin_lock *lock =
        manifest == NULL ? NULL : telos_plugin_lock_load(paths->lock, &error);
...
static bool config_operation(void *context)
{
    ...
    struct telos_config *config =
        telos_config_load(paths->home, paths->project, &error);
...
int main(int argc, char **argv)
{
    ...
    assert(snprintf(skill_directory, sizeof(skill_directory), "%s/skill",
                    root) < (int)sizeof(skill_directory));
    assert(snprintf(skill_path, sizeof(skill_path), "%s/SKILL.md",
                    skill_directory) < (int)sizeof(skill_path));
    assert(snprintf(home, sizeof(home), "%s/home", root) < (int)sizeof(home));
    assert(snprintf(telos_home, sizeof(telos_home), "%s/.telos", home) <
           (int)sizeof(telos_home));
    assert(snprintf(user_config, sizeof(user_config), "%s/config.toml",
                    telos_home) < (int)sizeof(user_config));
    assert(snprintf(project, sizeof(project), "%s/project", root) <
           (int)sizeof(project));
    assert(snprintf(project_config, sizeof(project_config), "%s/telos.toml",
                    project) < (int)sizeof(project_config));
    assert(snprintf(markdown_path, sizeof(markdown_path), "%s/events.md",
                    root) < (int)sizeof(markdown_path));
    ...
    write_text(user_config, "[agent]\nprovider = \"user-provider\"\nmodel = "
                            "\"user-model\"\n");
    write_text(project_config, "[builder]\nbackend = \"native\"\n");
