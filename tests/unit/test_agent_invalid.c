...
static bool silent_failure(const struct telos_tool_context *context,
                           const struct telos_value *arguments,
                           struct telos_value **result,
                           struct telos_error **error)
{
...
static bool explicit_failure(const struct telos_tool_context *context,
                             const struct telos_value *arguments,
                             struct telos_value **result,
                             struct telos_error **error)
{
...
static bool emit_completed(telos_provider_event_fn emit,
                           void *context,
                           struct telos_error **error)
{
...
static bool emit_call(const char *call_id,
                      const char *name,
                     ...
                     struct telos_error **error)
{
    ...
    case DISPATCH_CALL_NULL_PAYLOAD:
        result = emit_call(
            fixture->mode == DISPATCH_CALL_NULL_ID    ? NULL
            : fixture->mode == DISPATCH_CALL_EMPTY_ID ? ""
                                                      : "call",
            fixture->mode == DISPATCH_CALL_NULL_NAME    ? NULL
            : fixture->mode == DISPATCH_CALL_EMPTY_NAME ? ""
                                                        : "echo",
            fixture->mode == DISPATCH_CALL_NULL_PAYLOAD ? NULL : payload, emit,
            emit_context, error);
        break;
    case DISPATCH_CALL_DUPLICATE:
        result =
            emit_call("duplicate", "echo", payload, emit, emit_context, error);
        if (result) {
            result = emit_call("duplicate", "echo", payload, emit, emit_context,
                               error);
        }
        break;
    case DISPATCH_LATE:
    ...
    case DISPATCH_ENDLESS_TOOL:
        result = emit_call("call",
                           fixture->mode == DISPATCH_MISSING_TOOL  ? "missing"
                           : fixture->mode == DISPATCH_SILENT_TOOL ? "silent"
                           : fixture->mode == DISPATCH_ERROR_TOOL  ? "failure"
                                                                   : "echo",
                           payload, emit, emit_context, error);
        if (result) {
            result = emit_completed(emit, emit_context, error);
        }
        break;
...
static void assert_invalid(const struct telos_agent_options *options,
...
static void
assert_dispatch_failure(enum dispatch_mode mode,
...
int main(void)
{
    ...
    for (enum dispatch_mode mode = DISPATCH_NULL_EVENT;
         mode <= DISPATCH_PROVIDER_ERROR;
         mode = (enum dispatch_mode)(mode + 1)) {
        assert_dispatch_failure(
            mode, TELOS_PROVIDER_STATE_LOCAL, &options, &request,
            mode == DISPATCH_TEXT_NULL ? TELOS_ERROR_DOMAIN_MEMORY
    ...
    assert_dispatch_failure(DISPATCH_FALSE, TELOS_PROVIDER_STATE_LOCAL,
