...
static bool echo(const struct telos_tool_context *context,
...
int main(void)
{
    ...
    struct telos_value *text_schema =
        telos_value_new_object(text_schema_keys, text_schema_values, 1);
    ...
    struct telos_value *schema =
        telos_value_new_object(schema_keys, schema_values, 3);
    ...
    struct telos_capability_broker *broker = telos_capability_broker_create(
        capabilities, 1, evaluate, &policy, NULL);
    ...
    struct telos_value *arguments =
        telos_value_new_object(argument_keys, argument_values, 1);
    ...
    passed = passed &&
             telos_tool_execute(generation, broker, TELOS_EXECUTION_CORE,
                                "echo", arguments, NULL, &result, NULL) &&
             result != NULL &&
