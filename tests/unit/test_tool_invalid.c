...
evaluate(const struct telos_policy_request *request, void *context)
{
    (void)request;
    (void)context;
...
static bool return_value(const struct telos_tool_context *context,
...
static bool fail_silently(const struct telos_tool_context *context,
                          const struct telos_value *arguments,
                          struct telos_value **result,
                          struct telos_error **error)
{
...
static bool succeed_without_value(const struct telos_tool_context *context,
                                  const struct telos_value *arguments,
                                  struct telos_value **result,
                                  struct telos_error **error)
{
...
static bool fixture_create(struct fixture *fixture)
{
    ...
    transaction =
        telos_registry_transaction_begin(fixture->registry, "test.tools", NULL);
    if (transaction == NULL ||
    ...
    fixture->broker =
        telos_capability_broker_create(capabilities, 1, evaluate, NULL, NULL);
...
static bool execute(struct fixture *fixture,
                    const char *tool_id,
                    ...
                    struct telos_error **error)
{
    ...
    bool succeeded =
        telos_tool_execute(fixture->generation, fixture->broker, domain,
                           tool_id, arguments, cancel, &result, error);

    telos_value_release(result);
    return succeeded;
}

static bool rejects_schema(struct fixture *fixture,
                           ...
                           const char *arguments_text)
{
    ...
    fixture->definition.execute = return_value;
    rejected = schema != NULL && arguments != NULL &&
               !execute(fixture, "schema", TELOS_EXECUTION_CORE, arguments,
                        NULL, &error) &&
               error != NULL;
...
static bool accepts_schema(struct fixture *fixture,
                           const char *schema_text,
                           const char *arguments_text)
{
    struct telos_value *schema = json(schema_text);
    ...
    fixture->definition.execute = return_value;
    accepted =
        schema != NULL && arguments != NULL &&
        execute(fixture, "schema", TELOS_EXECUTION_CORE, arguments, NULL, NULL);
    telos_value_release(arguments);
    telos_value_release(schema);
    return accepted;
}
...
int main(void)
{
    ...
    passed =
        passed &&
        telos_capability_broker_create(NULL, 0, NULL, NULL, &error) == NULL &&
        error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             telos_capability_broker_create(bad_capabilities, 1, evaluate, NULL,
                                            &error) == NULL &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             telos_capability_broker_create(empty_capabilities, 1, evaluate,
                                            NULL, &error) == NULL &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             telos_capability_broker_create(empty_capabilities, SIZE_MAX,
                                            evaluate, NULL, &error) == NULL &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    passed = passed &&
             !telos_tool_execute(NULL, fixture.broker, TELOS_EXECUTION_CORE,
                                 "schema", empty, NULL, &result, &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             !telos_tool_execute(fixture.generation, NULL, TELOS_EXECUTION_CORE,
                                 "schema", empty, NULL, &result, &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed =
        passed &&
        !execute(&fixture, "", TELOS_EXECUTION_CORE, empty, NULL, &error) &&
        error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             !execute(&fixture, "missing", TELOS_EXECUTION_CORE, empty, NULL,
                      &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed &&
             !execute(&fixture, "schema", TELOS_EXECUTION_PROVIDER, empty, NULL,
                      &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    passed = passed &&
             !execute(&fixture, "schema", TELOS_EXECUTION_CORE, empty, NULL,
                      &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    passed =
        passed && rejects_schema(&fixture, "[]", "{}") &&
        rejects_schema(&fixture, "{\"type\":\"unknown\"}", "null") &&
        rejects_schema(&fixture, "{\"type\":\"object\"}", "[]") &&
        rejects_schema(&fixture, "{\"type\":\"object\",\"required\":{}}",
                       "{}") &&
        rejects_schema(&fixture, "{\"type\":\"object\",\"required\":[1]}",
                       "{}") &&
        rejects_schema(&fixture,
                       "{\"type\":\"object\",\"required\":[\"name\"]}", "{}") &&
        rejects_schema(&fixture, "{\"type\":\"object\",\"properties\":[]}",
                       "{}") &&
        rejects_schema(&fixture,
    ...
    passed = passed &&
             !execute(&fixture, "schema", TELOS_EXECUTION_CORE, empty, cancel,
                      &error) &&
             error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    telos_error_release(error);
    error = NULL;
    passed = passed && !execute(&fixture, "schema", TELOS_EXECUTION_CORE, empty,
                                NULL, NULL);
