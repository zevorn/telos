...
require_approval(const struct telos_policy_request *request, void *context)
{
    (void)request;
    (void)context;
...
static bool should_not_run(const struct telos_tool_context *context,
...
int main(void)
{
    ...
    struct telos_capability_broker *broker =
        telos_capability_broker_create(NULL, 0, require_approval, NULL, NULL);
    ...
    if (telos_tool_execute(generation, broker, TELOS_EXECUTION_PROVIDER,
                           "danger", arguments, NULL, &result, &error) ||
        error == NULL) {
    ...
    error = NULL;
    if (telos_tool_execute(generation, broker, TELOS_EXECUTION_CORE, "danger",
                           arguments, NULL, &result, &error) ||
