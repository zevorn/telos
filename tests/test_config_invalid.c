...
{
    struct telos_error *error = NULL;
    bool rejected = !telos_config_override(config, key, value, &error)
        && error != NULL;
...
int main(void)
{
    ...
    if (
        ...
        || snprintf(config_path, sizeof(config_path), "%s/telos.toml", project)
            < 0
        || mkdir(home, 0700) != 0
        || mkdir(telos_home, 0700) != 0
        || mkdir(project, 0700) != 0
    ) {
        return 1;
    }

    passed = telos_config_load(NULL, project, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && telos_config_load("relative", project, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && telos_config_load(home, NULL, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    passed = passed
        && telos_config_load(home, "relative", &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    passed = passed
        && telos_config_load(long_path, project, &error) == NULL
        && error != NULL;
    telos_error_release(error);
    error = NULL;
    ...
    passed = passed
        && config != NULL
        && strcmp(
            telos_config_get(config, "providers.openai.state_mode"),
            "remote"
        ) == 0
        ...
        && override_rejected(config, "providers.openai.secret", "plain")
        && override_rejected(config, "state.directory", "relative")
        && telos_config_override(
            config,
            "builder.backend",
            "native",
            NULL
