...
static bool create_rejected(struct telos_openai_responses_config config)
{
    ...
    struct telos_openai_responses_provider *provider =
        telos_openai_responses_provider_create(&config, &error);
...
{
    struct telos_error *error = NULL;
    bool rejected = !telos_openai_responses_provider_dispatch(
        request,
        callback,
        NULL,
        provider,
...
int main(void)
{
    ...
    struct telos_openai_responses_provider *provider;
    struct telos_error *error = NULL;
    bool passed = telos_openai_responses_provider_create(NULL, &error) == NULL
        && error != NULL;
    ...
    provider = telos_openai_responses_provider_create(&config, NULL);
    passed = passed
        && provider != NULL
    ...
    provider = telos_openai_responses_provider_create(&invalid, NULL);
    ...
    telos_openai_responses_provider_destroy(provider);
    telos_openai_responses_provider_destroy(NULL);
