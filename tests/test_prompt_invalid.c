...
static bool test_fragments(void)
{
    ...
    struct telos_error *error = NULL;
    struct telos_prompt_snapshot *empty = telos_prompt_snapshot_create(
        NULL,
        0,
        NULL
    );
    ...
    struct telos_prompt_snapshot *retained = telos_prompt_snapshot_retain(
        snapshot
    );
    const char *content = telos_prompt_snapshot_content(snapshot);
    bool passed = empty != NULL
        && snapshot != NULL
        && retained == snapshot
        && content != NULL
        && strstr(content, "policy-a") < strstr(content, "policy-z")
        && strstr(content, "policy-z") < strstr(content, "user")
        ...
        && strstr(
            telos_prompt_kernel_contract(),
            "Do not invent unavailable tools"
        ) != NULL
        && telos_prompt_snapshot_create(NULL, 1, &error) == NULL
        && error != NULL;
...
)
{
    ...
    char *project = (char *)1;
    struct telos_error *error = NULL;
    bool rejected = !telos_guidance_discover(
        home,
        root,
        current,
        &user,
...
static bool test_guidance(void)
{
    ...
    passed = telos_guidance_discover(
        home,
        root,
        nested,
        &user,
    ...
    error = NULL;
    passed = passed
        && discover_rejected(home, "/missing/telos-root", nested)
        && discover_rejected(home, root, outside)
        && write_large(user_file)
        && discover_rejected(home, root, nested);

    unlink(user_file);
    passed = passed
        && mkdir(root_file, 0700) == 0
        && discover_rejected(home, root, nested);
    rmdir(root_file);
    passed = passed
        && write_text(user_file, "")
...
int main(void)
{
    if (!test_fragments() || !test_guidance()) {
        fputs("Prompt validation or guidance failure matrix failed\n", stderr);
        return 1;
    }
    return 0;
}
